    ...
    try {
      if (i.customId === 'help_btn_portal') {
        const portalEmbed = new EmbedBuilder()
          ...
          .setThumbnail(client.user.displayAvatarURL())
          .setDescription(
            `Selamat datang di **Sentinel Portal Hub**! 2026\n\n` +
        ...
        await i.reply({ embeds: [portalEmbed], components: [row], flags: 64 });
      }
...
async function sendPortalHubDirect(replyTarget, isInteraction, user, guild, client) {
  ...
  if (isInteraction) {
    await replyTarget.reply({ embeds: [portalEmbed], components: [row], flags: 64 });
  } else {
