                ...
                try {
                  ...
                  itemCollector.on('collect', async iItemUse => {
                    ...
                    try {
                      ...
                      if (selectedItemId === 'PET_RENAME') {
                        const modal = new ModalBuilder()
                          .setCustomId('pet_modal_rename_inv')
                          .setTitle('🏷️ Ubah Nama Peliharaan Anda');

                        const nameInput = new TextInputBuilder()
                          .setCustomId('new_pet_name')
                          .setLabel('Nama Baru Peliharaan')
                          .setStyle(TextInputStyle.Short)
                          .setPlaceholder('Masukkan nama baru (Maks 25 karakter)...')
                          .setMinLength(1)
                          .setMaxLength(25)
                          .setRequired(true);
                        ...
                        await iItemUse.showModal(modal);

                        const submitted = await iItemUse.awaitModalSubmit({
                          time: 60000,
                          filter: (m) => m.user.id === user.id && m.customId === 'pet_modal_rename_inv'
                        }).catch(() => null);
    ...
    try {
      ...
      const getQuestText = (qType, progress, target) => {
        ...
        switch (qType) {
          ...
          case 'GARDEN_PLANT': descText = `Menanam benih bunga (\`.tanam\`) sebanyak 2 kali`; break;
          case 'GARDEN_HARVEST': descText = `Memanen bunga matang (\`.panen\`) sebanyak 2 kali`; break;
          case 'PVP_BOT': descText = `Bertanding di PvP Bot Arena (\`.pet pvpbot\`) sebanyak 2 kali`; break;
          default: descText = `Misi Harian`;
        }
        return `${descText} (${progress}/${target})`;
      };
  ...
  if (subCommand === 'raid' || subCommand === 'worldboss' || subCommand === 'boss') {
    ...
    try {
      const getRaidPanelData = (userId, gId, freshPet) => {
        ...
        const row1 = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId('pet_btn_raid_attack').setLabel('⚔️ Serang Boss (Solo)').setStyle(ButtonStyle.Danger).setDisabled(boss.status !== 'ACTIVE'),
          new ButtonBuilder().setCustomId('pet_btn_raid_soda').setLabel('🥤 Gunakan Soda (+1 Solo)').setStyle(ButtonStyle.Primary).setDisabled(boss.status !== 'ACTIVE'),
          new ButtonBuilder().setCustomId('pet_btn_close_panel').setLabel('❌ Tutup').setStyle(ButtonStyle.Secondary)
        );
        ...
        try {
          if (i.customId === 'pet_btn_close_panel') {
            ...
            try {
              ...
              await i.editReply({ content: `❌ **Pendaftaran Gagal:** ${err.message}` });
            }
          } else if (i.customId === 'pet_btn_raid_coop_start') {
            await i.deferReply();
            try {
              const fightRes = pet.executeWorldRaid(guildId);
              ...
              await i.editReply({ content: `❌ **Gagal Memulai Raid:** ${err.message}` });
            }
  ...
  if (subCommand === 'expedition' || subCommand === 'pet-expedition' || subCommand === 'expidition') {
    ...
    lobby.timeout = setTimeout(async () => {
      ...
      try {
        ...
        const s1EditOpts = {
          content: `🧭 **STAGE 1: PEMILIHAN JALUR TIM**\nKomandan <@${author.id}>, silakan pilih jalur ekspedisi untuk tim Anda!`,
          embeds: [],
        ...
        const s1dOpts = {
          content: `🧭 **STAGE 1 SELESAI:** Tim mengambil **${pathText.split('\n')[0]}**!`,
          embeds: [],
          components: [],
          attachments: []
        };
        ...
        const s2TransOpts = {
          content: `🚪 **[STAGE 2] Memasuki Area:** Kejadian Acak...`,
          embeds: [],
          components: [],
          attachments: []
        };
        ...
        if (isChest) {
          ...
          const s2cOpts = {
            content: `📦 **STAGE 2: PETI KUNO TERKUNCI**\nKomandan <@${author.id}>, pilih tindakan untuk membuka peti kuno!`,
            embeds: [],
            components: [chestRow],
            attachments: []
          };
          ...
          const s2wOpts = {
            content: `💧 **STAGE 2: AIR TERJUN SUCI**\nKomandan <@${author.id}>, pilih tindakan untuk memulihkan tim!`,
            embeds: [],
            components: [waterfallRow],
            attachments: []
          };
        ...
        const s2dOpts = {
          content: `📦 **STAGE 2 SELESAI:** Tim memilih **${eventText.split('\n')[0]}**!`,
          embeds: [],
          components: [],
          attachments: []
        };
        ...
        const s3TransOpts = {
          content: `🚪 **[STAGE 3] Memasuki Area:** Pertempuran Bos Akhir...`,
          embeds: [],
          components: [],
          attachments: []
        };
        ...
        for (let idx = 0; idx < totalSteps; idx++) {
          ...
          const qteOpts = {
            content: `⚔️ **BATTLE TURN ${stepNumber}/${totalSteps}!** Giliran <@${targetUserId}> untuk menyerang **${bossName}**! Cepat klik tombol di bawah sebelum waktu habis! <t:${endTimeUnix}:R>`,
            embeds: [],
            components: [qteRow],
            attachments: []
          };
          ...
          itemCollector.on('collect', async iItemUse => {
            ...
            try {
              ...
              if (selectedItemId === 'PET_RENAME') {
                ...
                const submitted = await iItemUse.awaitModalSubmit({
                  time: 60000,
                  filter: (m) => m.user.id === author.id && m.customId === 'pet_modal_rename_inv'
                }).catch(() => null);
