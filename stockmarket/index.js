...
/**
 * Handler untuk perintah Slot Machine (.slot) dengan animasi dinamis
 */
...
/**
 * Handler untuk perintah Toko Barang Mewah (.luxury)
 */
async function handleLuxuryCommand(message, client, args) {
  const { guildId, author } = message;
  const subCommand = args[0] ? args[0].toLowerCase() : null;
  ...
  if (subCommand === 'buy') {
    const itemId = args[1];
    const qty = args[2] ? parseInt(args[2]) : 1;

    if (!itemId) {
      const errEmb = embeds.errorEmbed('Format Salah! 🏎️', 'Gunakan: `.luxury buy <item_id> [jumlah]`\nContoh: `.luxury buy lambo 1`');
      return message.reply({ embeds: [errEmb] });
    }

    try {
      const res = luxury.buyLuxury(author.id, guildId, itemId, qty);
      const winEmb = embeds.successEmbed(
        'Pembelian Barang Mewah Sukses! 🏎️✨',
        `Selamat! Anda sukses membeli **${res.quantity}x ${res.item.name}**!\n` +
        `💰 **Total Pembayaran:** **Rp ${res.totalPrice.toLocaleString('id-ID')}** (Koin langsung dibakar!)\n` +
        `🎒 **Jumlah di Kantong:** **x${res.newQty}**\n\n` +
        `📉 **Sisa Saldo Dompet:** **Rp ${res.newBalance.toLocaleString('id-ID')}**`
      );
      return message.reply({ embeds: [winEmb] });
    } catch (err) {
  ...
  if (subCommand === 'sell') {
    ...
    if (!itemId) {
      const errEmb = embeds.errorEmbed('Format Salah! 🪙', 'Gunakan: `.luxury sell <item_id> [jumlah]`\nContoh: `.luxury sell rolex 1`');
      return message.reply({ embeds: [errEmb] });
    }

    try {
      const res = luxury.sellLuxury(author.id, guildId, itemId, qty);
      const sellPrice = Math.floor(res.item.price * 0.5);
      const winEmb = embeds.successEmbed(
        'Penjualan Barang Mewah Sukses! 💰',
        `Anda menjual **${res.quantity}x ${res.item.name}** kembali ke sistem!\n` +
        `💸 **Harga Jual Balik (Potong 50%):** Rp ${sellPrice.toLocaleString('id-ID')} / unit\n` +
        `💰 **Koin Diterima:** **Rp ${res.totalPayout.toLocaleString('id-ID')}**\n` +
        `🎒 **Sisa di Kantong:** **x${res.newQty}**\n\n` +
        `📉 **Saldo Dompet Baru:** **Rp ${res.newBalance.toLocaleString('id-ID')}**`
      );
      return message.reply({ embeds: [winEmb] });
    } catch (err) {
      const errEmb = embeds.errorEmbed('Penjualan Gagal!', err.message);
      return message.reply({ embeds: [errEmb] });
    }
  }

  if (subCommand === 'inv' || subCommand === 'inventory') {
    const inventory = luxury.getLuxuryInventory(author.id, guildId);
    
    let descText = '👑 **KOLEKSI BARANG MEWAH & PRESTISE SULTAN** 👑\n*Etalase barang-barang antik dan berharga fantastis miliki Anda.*\n────────────────────────────────────────\n\n';
    
    if (inventory.length === 0) {
      descText += '🚫 *Anda belum memiliki barang mewah apa pun di kantong Anda. Kumpulkan koin dan beli di `.luxury`!*';
    } else {
      inventory.forEach(item => {
        descText += `${item.name} (\`${item.id.toLowerCase()}\`) — **x${item.quantity}**\n` +
                    `├─ 🏷️ *“${item.desc}”*\n` +
                    `└─ 💵 *Nilai Resale: Rp ${(Math.floor(item.price * 0.5) * item.quantity).toLocaleString('id-ID')} (50% dari harga asli Rp ${item.price.toLocaleString('id-ID')})*\n\n`;
      });
    }
    ...
    const invEmbed = new EmbedBuilder()
      .setColor(0xD4AF37) // Metallic Gold Color!
      .setTitle(`💼 ETALASE PRESTISE — ${author.username}`)
      .setThumbnail(author.displayAvatarURL({ dynamic: true }))
      .setDescription(descText)
      .setFooter({ text: 'Barang mewah dapat dibeli dengan .luxury buy <id>' })
      .setTimestamp();

    return message.reply({ embeds: [invEmbed] });
  }

  // Dashboard Toko Barang Mewah Sultan (Katalog)
  const items = config.luxury.ITEMS || {};
  let catalogText = 'Selamat datang di **Luxury & Status Shop Kosan 1A**.\n' +
                    'Di sini kami menjual barang-barang prestise tinggi khusus bagi para warga terhormat dan sultan. Koin yang digunakan untuk membeli barang di sini akan langsung dibakar sistem dari sirkulasi server!\n\n' +
                    '**🛒 KATALOG BARANG MEWAH SULTAN:**\n\n';

  Object.values(items).forEach(item => {
    catalogText += `${item.name} (\`${item.id.toLowerCase()}\`)\n` +
                   `├─ 💵 **Harga Beli** : **Rp ${item.price.toLocaleString('id-ID')}**\n` +
                   `├─ 💸 **Resale (50%)**: Rp ${Math.floor(item.price * 0.5).toLocaleString('id-ID')}\n` +
                   `└─ 💬 *“${item.desc}”*\n\n`;
  });

  catalogText += '────────────────────────────────────────\n' +
                 `💡 *Ketik \`.luxury buy <item_id> [jumlah]\` untuk membeli barang.*\n` +
                 `💡 *Ketik \`.luxury inv\` untuk memamerkan barang milikmu di profil!*`;

  const luxEmbed = new EmbedBuilder()
    .setColor(0xD4AF37) // Gold!
    .setTitle('🏎️ LUXURY & STATUS SHOP KOSAN 1A')
    .setDescription(catalogText)
    .setFooter({ text: 'Sentinel Luxury Group • Prestige Redefined' })
    .setTimestamp();

  return message.reply({ embeds: [luxEmbed] });
}
...
async function handleEconomyCommands(message, client) {
  ...
  // ═══════════════════════════════════════════════════
  // Perintah: .luxury / .lux (Toko Barang Mewah)
  // ═══════════════════════════════════════════════════
  if (commandName === 'luxury' || commandName === 'lux') {
    await handleLuxuryCommand(message, client, args);
    return true;
  }
